...
%     \scalebox{0.9}{
% \begin{quantikz}
%   \lstick{$\mathcal{A}: \ket{0}$} & \gate{R} & \gate[wires=3]{U_E} & \qw & \midstick[3,brackets=none]{\dots} & \gate{R} & \gate[wires=3]{U_E} & \meter{} & & \rstick{$\ket{0}$} \\
%   \lstick{$\ket{0}$} & \gate{R} &  & \qw & & \gate{R} & & \meter{} & \rstick[wires=2]{output} \\
%   \lstick{$\ket{0}$} & \gate{R} &  & \qw & & \gate{R} & & \meter{} & \\
% \end{quantikz}}
%     \end{center}
% \end{frame}
